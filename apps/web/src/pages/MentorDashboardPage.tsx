    ...
    <AppShell>
      <div className="space-y-8">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.2fr,0.8fr]">
          <Card className="p-5"><h3 className="text-lg font-semibold text-white">Student progress and risk table</h3><p className="mt-2 text-xs text-slate-400 sm:hidden">Mobile summary cards are shown first. The full table scrolls sideways below.</p><div className="mt-4 grid gap-3 md:hidden">{cohortDashboard.students.map((student) => <div key={student.id} className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-white">{student.name}</p><p className="mt-1 text-sm text-slate-400">{student.goalPath}</p></div><Badge className={student.riskStatus === 'inactive' || student.riskStatus === 'struggling' ? 'border-rose-400/40 text-rose-200' : student.riskStatus === 'needs attention' ? 'border-amber-400/40 text-amber-200' : 'border-emerald-400/40 text-emerald-200'}>{student.riskStatus}</Badge></div><dl className="mt-3 grid grid-cols-3 gap-2 text-sm"><div><dt className="text-slate-500">Lessons</dt><dd className="text-white">{student.lessonsCompleted}</dd></div><div><dt className="text-slate-500">Quiz</dt><dd className="text-white">{student.quizAccuracy}%</dd></div><div><dt className="text-slate-500">Labs</dt><dd className="text-white">{student.labsCompleted}</dd></div></dl><p className="mt-3 text-sm text-slate-300">Next: {student.recommendedNextAction}</p></div>)}</div><div className="mt-4 overflow-x-auto" aria-label="Student progress table scroll area"><table className="mt-4 min-w-full text-left text-sm"><thead className="text-slate-400"><tr>{['Name','Goal/path','Lessons','Quiz','Labs','Artifacts','Last active','Risk','Next action'].map((head) => <th key={head} className="px-3 py-2 font-medium">{head}</th>)}</tr></thead><tbody>{cohortDashboard.students.map((student) => <tr key={student.id} className="border-t border-slate-800"><td className="px-3 py-3 text-white">{student.name}</td><td className="px-3 py-3 text-slate-300">{student.goalPath}</td><td className="px-3 py-3 text-slate-300">{student.lessonsCompleted}</td><td className="px-3 py-3 text-slate-300">{student.quizAccuracy}%</td><td className="px-3 py-3 text-slate-300">{student.labsCompleted}</td><td className="px-3 py-3 text-slate-300">{student.portfolioArtifacts}</td><td className="px-3 py-3 text-slate-300">{new Date(student.lastActiveAt).toLocaleDateString()}</td><td className="px-3 py-3"><Badge className={student.riskStatus === 'inactive' || student.riskStatus === 'struggling' ? 'border-rose-400/40 text-rose-200' : student.riskStatus === 'needs attention' ? 'border-amber-400/40 text-amber-200' : 'border-emerald-400/40 text-emerald-200'}>{student.riskStatus}</Badge></td><td className="px-3 py-3 text-slate-300">{student.recommendedNextAction}</td></tr>)}</tbody></table></div></Card>
        ...
        {cohortDashboard?.masteryHeatmap?.length ? (
          <Card className="p-5">
            ...
            <div className="mt-5 grid gap-3 md:hidden">{cohortDashboard.masteryHeatmap.map((row) => { const needsReview = row.skills.filter((skill) => skill.state === 'needs_review').length; const avg = Math.round(row.skills.reduce((sum, skill) => sum + skill.score, 0) / Math.max(1, row.skills.length)); return <div key={row.studentId} className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4"><p className="font-medium text-white">{row.studentName}</p><p className="mt-2 text-sm text-slate-300">Average mastery: {avg}% · Needs review: {needsReview}</p><div className="mt-3 h-2 rounded-full bg-slate-800"><div className="h-2 rounded-full bg-sky-300" style={{ width: `${avg}%` }} /></div></div>; })}</div><p className="mt-4 text-xs text-slate-400 md:hidden">Desktop heatmap is available below as a horizontal scroll table.</p><div className="mt-5 overflow-x-auto" aria-label="Cohort mastery heatmap scroll area">
